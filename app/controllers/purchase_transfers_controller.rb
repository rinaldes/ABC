class PurchaseTransfersController < ApplicationController
  ...
  def create
    ...
    @purchase_transfer = PurchaseTransfer.new(purchase_transfer_params.merge(status: ProductMutation::PENDING, mutation_date: Time.now,
      ...
      total_value: details.map{|mutation|(ProductSize.find_by_id(mutation["product_size_id"]).product.selling_price.to_f)*mutation["quantity"].to_i}.sum, sender_id: current_user.id,
      total_quantity: details.map{|mutation|mutation["quantity"].to_i}.sum, sell_price: details.map{|mutation|(ProductSize.find_by_id(mutation["product_size_id"]).product.selling_price)}.sum
    ))
    respond_to do |format|
      if @purchase_transfer.save
        ...
        if pr.present?
          ...
          pr_closed = false
          ProductMutationDetail.where("product_mutation_id IN (SELECT transfer_product_id FROM pr_transfer_products WHERE purchase_request_id=?) AND quantity>0", pr.id).group_by(&:product_size_id).each{|b|
          ...
          PrTransferProduct.create purchase_request_id: pr.id, transfer_product_id: @purchase_transfer.id, receiving_id: (rc.id rescue nil), stock_ho: rc.received_qty - details.map{|a|a['quantity'].to_i}.sum
          pr.update_attributes(status: pr_closed ? 'CLOSED' : PurchaseRequest::TRANSFER_ON_PROGRESS)
        else
          rc = Receiving.find_by_number(params[:receive_number]) || Receiving.where("distributed_qty<received_qty").limit(1).first
          PrTransferProduct.create transfer_product_id: @purchase_transfer.id, receiving_id: (rc.id rescue nil), stock_ho: rc.received_qty - details.map{|a|a['quantity'].to_i}.sum
        end
        rc.update_attributes(distributed_qty: rc.distributed_qty+@purchase_transfer.total_quantity)
  ...
  private
    def purchase_transfer_params
      params.require(:purchase_transfer).permit(
        :origin_warehouse_id, :ongkir, :code, :destination_warehouse_id, :status, :no_surat_jalan, :note, :supplier_id, product_mutation_details_attributes: [:quantity, :product_size_id]
      )
    end
