class ArVouchersController < ApplicationController
  ...
  def create
    @branches = Branch.select("office_name, office_name").limit(11).order("office_name").map{|product|[product.office_name, product.office_name]}
    @ar_voucher = ArVoucher.new(ar_voucher_params.merge(max_voucher_amt: params[:floated_max_voucher_amt], code: params[:code]))
    if @ar_voucher.save
      flash[:notice] = 'AR Voucher berhasil ditambahkan'
  ...
  def update
    ...
    if @ar_voucher.update_attributes(ar_voucher_params.merge(max_voucher_amt: params[:floated_max_voucher_amt]))
      ArVoucherPaymentSchedule.where(id: old_schedules).delete_all
      flash[:notice] = 'ArVoucher was successfully updated.'
      redirect_to ar_vouchers_path
    else
