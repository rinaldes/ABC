class ReturnsToHosController < ApplicationController
  ...
  def print_to_pdf
    ...
    send_data(pdf, 
      :filename    => "Return to HO #{@good_transfer.code}.pdf", 
  ...
  def create
    ...
    @returns_to_ho = ReturnsToHo.new(
      returns_to_ho_params.merge(
        ...
        total_price: details.map{|mutation|(ProductSize.find_by_id(mutation["product_size_id"]).product.cost_of_products*mutation["quantity"].to_i)}.sum,
        sender_id: current_user.id, total_quantity: details.map{|mutation|mutation["quantity"].to_i}.sum,
        sell_price: details.map{|mutation|(ProductSize.find_by_id(mutation["product_size_id"]).product.selling_price*mutation["quantity"].to_i)}.sum
      )
    )
    respond_to do |format|
  ...
  def get_product
    ...
    @list_product = ProductDetail.where("warehouse_id=#{(params[:origin_warehouse_id])} AND products.id=#{@product.id}").joins(product_size: [:product])
...
private
  def returns_to_ho_params
    params.require(:returns_to_ho).permit(:batal_retur, :ganti_barcode, :code, :note, :receive_no, :supplier_id, :no_surat_jalan, :supplier_id, product_mutation_details_attributes: [:quantity, :product_size_id])
  end
