class Finances::CashOutsController < ApplicationController
  ...
  def update
    ...
    @cash_out.update_attributes(description: params[:cash_out][:description])
  ...
  def show
    if params[:date].present?
      ...
      @petty_cash.generate_init_budget if @petty_cash.petty_cash_details.first.initial_budget.blank?
  ...
  private
    def cash_out_params
      params.require(:cash_out).permit(:amount, :cash_out_type, :description, :petty_cash_id, :unit, :qty, :price_per_unit)
    end
end
