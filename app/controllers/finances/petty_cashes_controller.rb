class Finances::PettyCashesController < ApplicationController
  ...
  def create
    if params[:f_init_budget].present?
      init_budget = params[:f_init_budget]
    else
      if params[:f_month].to_i == 1
        init_budget = PettyCash.where(year: "#{params[:f_year].to_i-1}-12-01", month: "#{params[:f_year].to_i-1}-12-01", office_id: params[:f_store]).first.petty_cash_details.order("date DESC, created_at DESC").first.balance rescue 0
      else
        init_budget = PettyCash.where(year: "#{params[:f_year].to_i}-#{params[:f_month].to_i-1}-01", month: "#{params[:f_year].to_i}-#{params[:f_month].to_i-1}-01", office_id: params[:f_store]).first.petty_cash_details.order("date DESC, created_at DESC").first.balance rescue 0
      end
    end
    begin
      PettyCash.generate_petty_cashes(params[:f_year], params[:f_month], params[:f_store], init_budget)
      prepare_petty_cashes
      flash[:notice] = 'Initial Petty Cash Successfully created.'
    rescue
      prepare_petty_cashes
      flash[:error] = 'Year already been taken'
    end
  end
  ...
  def show
    ...
    @cash_outs = CashOut.where(conditions.join(' AND ')).where(petty_cash_id: params[:id]).order("created_at ASC")#.page(params[:page]).per(PER_PAGE)
    @office = Office.find @petty_cash.office_id
  end
