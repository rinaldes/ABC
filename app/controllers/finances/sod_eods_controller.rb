class Finances::SodEodsController < ApplicationController
 ...
 private
  ...
  def sod_eod_params
    params.require(:sod_eod).permit(
      :dp_masuk, :dp_keluar, :tombokan, :dp, :cash2, :lain_lain, :input_service,:office_id, :start_time, :end_time, :actual_end_amount, :note, :user_id, cash_outs_attributes: [:id, :time, :description, :amount,
      :_destroy, :receipt], cash_at_banks_attributes: [:id, :sod_eod_id, :edc_bank_account_id, :saving_amount, :_destroy, :saving_date, :required_amount])
  end
