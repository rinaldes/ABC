class DepartmentsController < ApplicationController
  ...
  def create
    @department = Department.new(department_params.merge(code: "M#{(MClass.where("parent_id IS NULL AND code IS NOT NULL").order("id DESC").limit(1).last.code.gsub('M', '').to_i rescue 0)+1}"))
    if @department.save
      flash[:notice] = 'Department berhasil ditambahkan'
      redirect_to departments_path
    else
