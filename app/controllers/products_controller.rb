...
#      redirect_to products_path
  ...
  def new
    ...
    @product = Product.new status4: 'Slow Moving', status1: 'Active', status2: 'Catalog', status5: 'Normal Brand'
    load_master_data_for_goods
    @product.branch_prices.build
    @catalog = []
    respond_to do |format|
      format.html
      format.js
...
# percent: params[:discount_percent]) if params[:discount].present? && [Discount::Normal, Discount::Fiftyfifty, Discount::TanggungSupplier, Discount::TidakAdaDiskon].include?(params[:discount][:discount_type]
  ...
  def create
    key = params[:key]
    mintam = params[:product][:branch_prices_attributes][key][:additional_min_stock] rescue nil
    branch_id = params[:product][:branch_prices_attributes][key][:branch_id] rescue nil
    brand = Brand.find_by_id(params[:product][:brand_id])
    params[:product][:purchase_price] = params[:product][:purchase_price].to_f
    params[:product][:selling_price] = params[:product][:selling_price].to_s.split('.').join("")
    colour = Colour.find_by_name(params[:colour1])
    unit = Unit.find_by_id(params[:product][:unit_id])
    @size = Size.find_by_id(params[:product][:size_id])
    ...
    m_class = MClass.find_by_id(params[:m_class_id3]) || MClass.find_by_id(params[:m_class_id2])
    @product = Product.new((params.require(:product).permit(
      :article, :cost_of_products, :selling_price, :purchase_price, :harga_bandrol, :harga_eceran, :harga_member_eceran, :harga_kredit, :image, :status1, :status2, :status3, :first_po, :margin_percent1,
      :margin_percent2, :margin_percent3, :margin_percent4, :margin_rp, :colour_code, :discount_percent, :margin_percent, :status5,
      product_sizes_attributes: [:size_detail_id, :default_min_stock, product_details_attributes: [[:min_stock, :available_qty, :allocated_qty, :freezed_qty,:rejected_qty, :online_qty, :defect_qty]]],
      branch_prices_attributes: [:branch_id, :percentage, :nominal, :additional_min_stock]
    ).merge(colour_id: (colour.id rescue nil), brand_id: (brand.id rescue nil), unit_id: (unit.id rescue nil), m_class_id: (m_class.id rescue ''), code: code,
      size_id: (@size.id rescue nil), colour2_id: (Colour.find_by_name(params[:colour2]).id rescue nil), colour3_id: (Colour.find_by_name(params[:colour3]).id rescue nil),
      colour4_id: (Colour.find_by_name(params[:colour4]).id rescue nil),
      ...
      department_m_class: set_department_m_class(m_class), department_id: (brand.department_id rescue nil), purchase_price: params[:floated_purchase_price], cost_of_products: params[:floated_purchase_price],
      selling_price: params[:floated_selling_price], harga_bandrol: params[:floated_harga_bandrol], harga_eceran: params[:floated_harga_eceran], harga_member_eceran: params[:floated_harga_member_eceran],
      harga_kredit: params[:floated_harga_kredit], margin_rp: params[:floated_margin_rp])
    ))

    respond_to do |format|
      if @product.save
        if params[:status2].present?
          params[:status2].each do |cat_id|
            ProductCatalog.create(product_id: @product.id, catalog_id: cat_id)
          end
        end
        product_size = ProductSize.where(product_id: @product.id)
        product_size.each{|ps|
        begin
          product_d = ProductDetail.where(product_size_id: ps.id)
          min_stock_old = ps.default_min_stock
          mini_stock = min_stock_old + mintam.to_i
          @pd = ProductDetail.find_by_product_size_id_and_warehouse_id(ps.id,branch_id)
          @pd.min_stock = mini_stock.to_i
          @pd.save
        rescue
        end
        }
  ...
  def update
    ...
    m_class = MClass.find_by_id(params[:m_class_id3]) || MClass.find_by_id(params[:m_class_id2])
    @product = Product.find(params[:id])
    code = @product.barcode[12..15]
    respond_to do |format|
      if @product.update_attributes(params.require(:product).permit(
        :article, :image, :status1, :status2, :status3, :first_po, :margin_percent1, :margin_percent2, :margin_percent3, :margin_percent4, :margin_rp, :colour_code, :discount_percent, :margin_percent, :status5,
          branch_prices_attributes: [:branch_id, :percentage, :nominal, :additional_min_stock]
      ).merge(colour_id: (colour.id rescue nil), brand_id: (brand.id rescue nil), unit_id: (unit.id rescue nil), m_class_id: (m_class.id rescue ''), size_id: (@size.id rescue nil),
        colour2_id: (Colour.find_by_name(params[:colour2]).id rescue nil), colour3_id: (Colour.find_by_name(params[:colour3]).id rescue nil), colour4_id: (Colour.find_by_name(params[:colour4]).id rescue nil),
        barcode: [(brand.supplier.code.gsub('S', '') rescue ''), (m_class.parent(1).code.reverse[0..1].reverse rescue ''), (m_class.parent(2).code[0..5] rescue ''), code].compact.join(''),
        department_m_class: set_department_m_class(m_class), department_id: (brand.department_id rescue nil), purchase_price: params[:floated_purchase_price], cost_of_products: params[:floated_purchase_price],
        selling_price: params[:floated_selling_price], harga_bandrol: params[:floated_harga_bandrol], harga_eceran: params[:floated_harga_eceran], harga_member_eceran: params[:floated_harga_member_eceran],
        harga_kredit: params[:floated_harga_kredit], margin_rp: params[:floated_margin_rp])
      )
        params[:product][:product_sizes_attributes].each{|ps|
          current_ps = @product.product_sizes.find_by_size_detail_id(ps[1][:size_detail_id])
          current_ps.update_attributes(default_min_stock: ps[1][:default_min_stock]) rescue ''
        }
        params[:product][:product_sizes_attributes].each{|product_size|
          ProductDetail.where(product_size_id: product_size[0]).update_all("min_stock=#{product_size[1]['product_details_attributes'][product_size[0]].values[0]}") rescue nil
        }
        if @product.harga_bandrol != params[:product][:harga_bandrol].to_f
          PriceHistory.create(harga_lama: @product.harga_bandrol, harga_baru: params[:product][:harga_bandrol], tipe_harga: "harga_bandrol", product_id: params[:id])
        end
        if @product.harga_eceran != params[:product][:harga_eceran].to_f
          PriceHistory.create(harga_lama: @product.harga_eceran, harga_baru: params[:product][:harga_eceran], tipe_harga: "harga_eceran", product_id: params[:id])
        end
        if @product.harga_member_eceran != params[:product][:harga_member_eceran].to_f
          PriceHistory.create(harga_lama: @product.harga_member_eceran, harga_baru: params[:product][:harga_member_eceran], tipe_harga: "harga_member_eceran", product_id: params[:id])
        end
        if @product.harga_kredit != params[:product][:harga_kredit].to_f
          PriceHistory.create(harga_lama: @product.harga_kredit, harga_baru: params[:product][:harga_kredit], tipe_harga: "harga_kredit", product_id: params[:id])
        end
        flash.now[:notice] = 'Product successfully updated.'
        format.html{redirect_to products_url}
      else
