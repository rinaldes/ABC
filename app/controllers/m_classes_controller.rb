class MClassesController < ApplicationController
  ...
  def create
    parent = MClass.find_by_id(params[:m_class][:parent_id])
    @m_class = MClass.new m_class_params.merge(level: 1, code: (("#{parent.code}#{[*('A'..'Z')][MClass.where("parent_id=#{parent.id}").count]}")))
    if @m_class.save
      @m_class.set_path
  ...
  def update_sub_mclass sub_m_classes, parent
    sub_m_classes.values.each_with_index{|m_class_params, i|
      ...
      if m_class
        ...
        lv = parent.level+1
        if lv == 2
          last_mclass_code = (MClass.where("level=2 AND parent_id=#{parent.id}").limit(1).order("code DESC").last.code.chop.chop.chop.to_i rescue 0)
          code = "#{sprintf '%03d', last_mclass_code}#{sprintf '%03d', i+1}"
        else
          code = "#{parent.code}.#{i+1}"
        end
        m_class = MClass.new name: m_class_params[:name], level: lv, parent_id: parent.id, code: code
        unless m_class.save
          @errors << m_class.errors.full_messages.join('<br/>')
          @quit = true
        end
  ...
  def create_sub_mclass sub_m_classes, parent
    ...
    sub_m_classes.values.each_with_index{|m_class_params, i|
      lv = parent.level+1
      if lv == 2
        code = "#{sprintf '%03d', last_mclass_code+1}#{sprintf '%03d', i+1}"
      else
        code = "#{parent.code}.#{i+1}"
      end
      m_class = MClass.new name: m_class_params[:name], level: lv, parent_id: parent.id, code: code
  ...
  def destroy
    ...
    if @m_class.check_transaction && @m_class.destroy
      flash[:error] = nil
      flash[:notice] = "Successfully delete mclass"
    else
      flash[:notice] = nil
      flash[:error] = "Can't delete mclass. data in use"
    end
...
private
  def get_paginated_m_class
    conditions = ["m_classes.level=1 AND m_classes.parent_id IS NOT NULL"]
    params[:search].each{|param|
      conditions << "LOWER(#{param[0]}) LIKE LOWER('%#{param[1]}%')"
    } if params[:search].present?
    @m_classes = MClass.where(conditions.join(' AND ')).order("TRIM(both 'M' from parent.code)::integer ASC").joins('INNER JOIN m_classes parent ON parent.id=m_classes.parent_id').paginate(:page => params[:page], :per_page => 10).select("m_classes.*, m_classes.name as name1, m_classes.name as name2, m_classes.code as code1, m_classes.code as code2, m_classes.path_code, m_classes.created_at") || []
  end

  def m_class_params
    params.require(:m_class).permit(:code, :name, :parent_id)
  end
