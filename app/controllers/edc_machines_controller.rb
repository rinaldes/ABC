class EdcMachinesController < ApplicationController
  ...
  def import
    if params[:file].present?
      if %w(text/csv application/vnd.ms-excel).include?params[:file].content_type
        ...
        flash[:notice] = "Supplier imported."
      else
  ...
  def get_paginated_edc_machines
    ...
    @edc_machines = EdcMachine.joins("LEFT JOIN offices ON edc_machines.store_id=offices.id").select("edc_machines.*, office_name").where(conditions.join(' AND ')).order(default_order('edc_machines')).paginate(page: params[:page], per_page: 10) || []
  end

  def edc_machine_params
    params.require(:edc_machine).permit(:store_id, :bank_name, :branch, :edc_serial_number, :account_number, :owner)
  end
