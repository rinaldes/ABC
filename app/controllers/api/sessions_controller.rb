...
=end
    ...
    if resource && resource.valid_password?(params[:password])
      ...
      render json: {token_string: api_key.access_token, login_time: time_now.iso8601, id: current_user.id}, status: 200
    else
      render json: {token_string: '', login_time: Time.now.iso8601, id: 0}, status: 200
    end
  end
end
