class MembersController < ApplicationController
  ...
  def show
    ...
    @transaction_histories = Sale.where(conditions.join(' AND ')).order(default_order('sales')) || []
  end
  ...
  def create
    ...
    if @member.save
      flash[:notice] = "Member Berhasil ditambahkan."
      redirect_to members_path
    else
  ...
  def update
    ...
    if @member.update_attributes(member_params)
      flash[:notice] = "Member berhasil diperbaharui."
      redirect_to members_path
    else
      flash[:error] = @member.errors.full_messages.join('<br/>')
      render "edit"
  ...
  def create_setting_point
    ...
    respond_to do |format|
      if @setting.save
        @member = "Diskon Member Berhasil ditambahkan."
        format.js
      else
  ...
  def create_member_discount
    ...
    respond_to do |format|
      ...
      if @discount.save
        @discount = Discount.new
        flash.now[:notice] = "Diskon Member Berhasil ditambahkan."
        format.js
      else
  ...
  def update_member_discount
    ...
    respond_to do |format|
      ...
      if @discount.update_attributes(params[:discount])
        @discount = Discount.new
        flash.now[:notice] = "Diskon Member berhasil diperbaharui."
        format.js
      else
        flash.now[:error] = @discount.errors.full_messages
        format.js
  ...
  def delete_member_discount
    ...
    respond_to do |format|
      get_paginated_discount_member
      @discount = Discount.new
      flash.now[:notice] = "Diskon Member berhasil dihapus"
      format.js
    end
  end
  ...
  def expire
    ...
    if @member.reactivate
      flash[:notice] = "Member berhasil diaktifasi kembali."
      redirect_to members_path
    else
      respond_to do |format|
        flash.now[:error] = @member.errors.full_messages
  ...
  private
  def member_params
    params.require(:member).permit(:code, :member_type_id, :card_number, :name, :birthday, :address, :hp, :email, :gender)
  end
