class MclassesController < ApplicationController
 ...
 def index
  ...
  def create
    ...
    if @mclass.save
      flash[:notice] = 'M-Class berhasil ditambahkan'
  ...
  def update
    ...
    if @mclass.update_attributes(mclass_params)
      flash[:notice] = 'M-Class was successfully updated.'
      redirect_to mclasses_path
    else
      flash[:error] = @mclass.errors.full_messages
      # format.js
...
=begin
    ...
    @mclass.destroy
    respond_to do |format|
      if @mclass.update_attributes(:available_id => true)
        flash.now[:notice] = 'M-Class berhasil dihapus.'
      else
        flash.now[:error] = 'M-Class tidak dapat dihapus karena sudah ada dipakai.'
      end
      format.js
    end
=end
