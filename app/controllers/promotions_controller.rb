class PromotionsController < ApplicationController
  ...
  def search_product_manual
    ...
    conditions << "LOWER(colours.name) LIKE LOWER('%#{params[:colour_name]}%')" if params[:colour_name].present?
    conditions << "LOWER(barcode) NOT IN (#{params[:present_product]})" if params[:present_product].present?
    @products = Product.joins(:brand, :colour).where(conditions.join(' AND ')).paginate(page: params[:page], per_page: 10) || []
  ...
  def create
    if params[:promotion][:promotion_type] == "Discount Period"
      params[:promotion][:promo_item_lists_attributes] = params[:promotion][:prize_lists_attributes]
    end
    if params[:promotion][:office_id] == 'All Store'
      Branch.all.each do |brch|
        @promotion = Promotion.create(promotion_params.merge(code: (('%03d' % ((Promotion.last.code.to_i rescue 0)+1))), office_id: brch.id))
      end
    else
      @promotion = Promotion.create(promotion_params.merge(code: (('%03d' % ((Promotion.last.code.to_i rescue 0)+1)))))
    end
    respond_to do |format|
      if @promotion.valid?
  ...
  def destroy
    ...
    if @promotion.destroy
      PrizeList.where(promotion_id: params[:id]).delete_all
      PromoItemList.where(promotion_id: params[:id]).delete_all
    end
    get_paginated_promotions
    respond_to do |format|
      format.js
...
private
  ...
  def promotion_params
    params.require(:promotion).permit(
      ...
      prize_lists_attributes: [:department_id, :m_class_id, :brand_id, :article_id, :max_qty, :product_id, :disc_amt, :prize_promo]
    )
  end
