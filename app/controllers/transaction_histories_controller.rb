class TransactionHistoriesController < ApplicationController
  ...
  def show
    ...
    @sales_details = @transaction_history.sales_details
    respond_to do |format|
      format.html
    end
  end
...
private
  def get_transaction_histories
    ...
    @transaction_histories = Sale.where(conditions.join(' AND ')).order(default_order('sales')).joins("LEFT JOIN members ON members.id=sales.member_id").paginate(:page => params[:page], :per_page => 10) || []
  end
end