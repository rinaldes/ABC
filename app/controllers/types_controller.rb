class TypesController < ApplicationController
  ...
  def create
    ...
    if @type.save
      ...
      flash[:notice] = 'Jenis berhasil ditambahkan.'
  ...
  def update
    ...
    if @type.update_attributes(type_params)
      ...
      flash[:notice] = 'Jenis berhasil diupdate.'
      redirect_to types_path
    else
      flash[:error] = @type.errors.full_messages
      render "edit"
  ...
  def destroy
    ...
    respond_to do |format|
      if @type.destroy
        ...
        flash.now[:notice] = 'Jenis berhasil dihapus.'
        @type = Type.new
        get_paginated_types
      else
        flash.now[:error] = 'Jenis tidak dapat dihapus karena sudah ada dipakai.'
      end
      format.js
    end
  end
