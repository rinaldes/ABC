class MemberLevelsController < ApplicationController
  ...
  def update
    ...
    if @member_level.update_attributes(member_level_params)
      flash[:notice] = 'MemberLevel was successfully updated.'
      redirect_to member_levels_path
    else
...
=begin
    ...
    respond_to do |format|
      if @member_level.update_attributes(:available_id => true)
        flash.now[:notice] = 'Warna berhasil dihapus.'
      else
        flash.now[:error] = 'Warna tidak dapat dihapus karena sudah ada dipakai.'
      end
      format.js
    end
=end
...
private
  def member_level_params
    params.require(:member_level).permit(:level, :description, :minimum_amount, :discount_percent)
  end
