class PoHistoriesController < ApplicationController
  ...
  private
    ...
    def get_paginated_purchase_orders
      conditions = ["offices.type='Branch'"]
      params[:search].each{|param|
        conditions << "LOWER(#{param[0]}) LIKE LOWER('%#{param[1]}%')" if param[1].present?
      } if params[:search].present?
      @product_details = ProductDetail.select("article, m_classes.name, suppliers.name AS supplier_name, offices.office_name AS store_name, warehouse_id, product_size_id")
        .joins(product_size: [product: [:m_class, brand: :supplier]]).joins("INNER JOIN offices ON offices.id=product_details.warehouse_id").limit(100).where(conditions.join(' AND '))
      @between = "BETWEEN '#{params[:start_date]} 00:00:00' AND '#{params[:end_date]} 23:59:59'"
    end
