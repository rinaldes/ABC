module ApplicationHelper
  ...
  def get_sum_total_receiving(receivings)
    Receiving.get_sum_total_receiving(receivings)
  end

  def get_amount_of_other_charges(account_payable_ids, other_charge_name)
    OtherCharge.get_amount_by_account_payable_ids_and_name(account_payable_ids, other_charge_name)
  end

  def get_total_price(branch_id, date)
    Sale.get_total_price_by_branch_id_and_date(branch_id, date)
  end

  def get_total_price_per_month(branch_id, dates)
    Sale.get_total_price_per_month(branch_id, dates)
  end

  def get_another_income(date, total)
    catalogue = 0
    jasa_kado = 0
    charge_merchant = 0
    pembulatan = 0
    salah_transaksi = 0
    dp_masuk = 0
    total += catalogue + jasa_kado + charge_merchant + pembulatan + salah_transaksi + dp_masuk
    hash_another_income = { "catalogue" => catalogue, "jasa_kado" => jasa_kado, "charge_merchant" => charge_merchant, "pembulatan" => pembulatan, "salah_transaksi" => salah_transaksi, "dp_masuk" => dp_masuk }
  end

  def get_payment_merchant(date, bank_name)
    SalesPaymentBank.get_payment_merchant(date, bank_name)
  end

  def get_payment_transfer(date, bank_name)
    SalesPaymentBank.get_payment_transfer(date, bank_name)
  end

  def count_cash_outs(cash)
   cash_in = cash.where(cash_out_type: "CASH IN").sum(:amount)
   cash_out = cash.where.not(cash_out_type: "CASH IN").sum(:amount)
   format_currency(cash_in.to_f - cash_out.to_f) 
  end
