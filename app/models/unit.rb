class Unit < ActiveRecord::Base
  ...
  validates :name, :short_name, presence: {message: 'harus diisi'}, :uniqueness => {:case_sensitive => false}
  validates_format_of :name, :with => /^[a-zA-Z\d\s]*$/i, :multiline => true, :message => "hanya mengijinkan input alphanumeric"
  validates_format_of :short_name, :with => /^[a-zA-Z\d\s]*$/i, :multiline => true, :message => "hanya mengijinkan input alphanumeric"
