class StockTransfer < ActiveRecord::Base
  has_many :stock_transfer_details

  belongs_to :user
  belongs_to :destination, :class_name => "Office", foreign_key: 'branch_id'
  ...
  def restore_quantity
    stock_transfer_details.each{|pmd|
      origin = ProductDetail.find_by_warehouse_id_and_product_size_id(branch_id, pmd.product_detail_id)
        ...
        if detail[1]["quantity"].present? && detail[1]["quantity"].to_i != 0
          ...
          if stock_transfer_detail
            ...
            ProductMutationHistory.create product_detail_id: origin.id, old_quantity: eval("origin.#{origin_stock.downcase}_qty.to_i"), moved_quantity: detail[1]["quantity"].to_i,
              ...
              product_mutation_detail_id: stock_transfer_detail.id, quantity_type: "#{origin_stock.downcase}_qty", id: (ProductMutationHistory.last.id rescue 0)+1
            ...
            unless destination_stock == 'Scrap'
              ProductMutationHistory.create product_detail_id: origin.id, old_quantity: eval("origin.#{destination_stock.downcase}_qty.to_i"),
                ...
                mutation_type: 'Stock Transfer', product_mutation_detail_id: stock_transfer_detail.id, quantity_type: "#{destination_stock.downcase}_qty", id: (ProductMutationHistory.last.id rescue 0)+1
