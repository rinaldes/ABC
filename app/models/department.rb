class Department < MClass
  ...
  has_many :suppliers, class_name: 'Supplier', through: :supplier_departments
  #has_many :suppliers, class_name: 'MClass', through: :supplier_departments
  validates_format_of :name, :with => /^[a-zA-Z\d\s]*$/i, :multiline => true, :message => "hanya mengijinkan input alphabet", uniqueness: true
  ...
  def self.import(file)
    ...
    CSV.foreach(file.path, headers: true) do |row|
      ...
      if find_by_name(row["name"].upcase).nil?
        ...
        department = new(parameters.permit(:name).merge(code: "M#{(MClass.where("parent_id IS NULL").order("id DESC").limit(1).last.code.gsub('M', '').to_i rescue 0)+1}"))
