class MemberLevel < ActiveRecord::Base
  ...
  def self.to_csv(options = {})
    CSV.generate(options) do |csv|
      ...
      all.each do |member_level|
        csv << [member_level.description, member_level.level, member_level.minimum_amount, member_level.discount_percent]
      end
    end
  end
  ...
  def self.import(file)
    ...
    if find_by_description(row["description"].upcase).nil?
      ...
      member_level.update(parameters.permit(:description, :level, :minimum_amount, :discount_percent))
      member_level.save
    else
