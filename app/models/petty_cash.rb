class PettyCash < ActiveRecord::Base
  ...
  def generate_init_budget
    @petty_cash = PettyCash.where("office_id=#{self.office_id} AND year='#{(year-1.month).strftime('%Y-%m-%d')}'").order("created_at ASC").last
    sisa_saldo = @petty_cash.petty_cash_details.first.initial_budget
    @cash_outs = @petty_cash.cash_outs
    ...
    self.petty_cash_details.first.update_attributes(initial_budget: sisa_saldo)
  end
