class StockOpname < ActiveRecord::Base
  ...
  def self.number
    number = (StockOpname.order("opname_number ASC").last.opname_number.last(6).to_i rescue 0) + 1
    return number
  end
  ...
  protected
    def check_details
      self.errors.add(:stock_opname_detail, "tidak boleh kosong") and return false unless self.stock_opname_details.present?
  	end
