class ProductMutation < ActiveRecord::Base
  ...
  def check_stock
    product_mutation_details.each{|p|
      #errors.add(:quantity, " in size #{p.product_size.size_detail.size_number} can't be more than available quantity") if p.product_size.size_detail.present? && p.quantity.to_i > ProductDetail.find_by_warehouse_id_and_product_size_id(origin_warehouse_id, p.product_size_id).available_qty.to_i
    }
  end
  ...
  def transfer_product_acceptance details
    self.update_attributes(status: ProductMutation::RECEIVED)
    details.each{|gtd|
      if gtd[1].present?
        pmd = ProductMutationDetail.where(product_mutation_id: id, product_size_id: gtd[1]['product_size_id']).first
        pmd.product_received_by_destination_branch
      end
    }
  end
