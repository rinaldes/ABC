<% if @product.present? %>
  ...
  <% @list_product.each_with_index do |detail, i|%>
    ...
    $("#table_size tbody tr:eq(2) td:eq(<%= i %>)").html("<%= @list_product_in_branch.find_by_product_size_id(detail.product_size_id).min_stock.to_i rescue 0 %>")
    $("#table_size tbody tr:eq(3) td:eq(<%= i %>)").html("<%= @list_product_in_branch.find_by_product_size_id(detail.product_size_id).available_qty.to_i rescue 0 %>")
    $("#table_size tbody tr:eq(4) td:eq(<%= i %>)").html("<%= j(text_field_tag ('quantity_number_' + (detail.product_size.size_detail.size_number.to_s rescue '')), '', class: 'quantity') %><input type='hidden' value='<%= detail.product_size_id %>'>")
  <% end %>
<% else %>
