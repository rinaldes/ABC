<% if @product.present? %>
  ...
  <% @list_product.each_with_index do |detail, i|%>
    ...
    $("#table_size tbody tr:eq(3) td:eq(<%= i %>)").html("<%= j(text_field_tag ('quantity_number_' + (detail.product_size.size_detail.size_number.to_s rescue ''))) %><input type='hidden' value='<%= detail.product_size_id %>'>")
  <% end %>
  nomor_ganti()
<% else %>
